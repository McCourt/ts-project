...
ggplot(data = df, aes(x = Stock, y = diff)) +
  geom_boxplot()
...
Diff_t &= ARIMA_{(p, q, d) \times (P, Q, D)_s}(Diff_{t-1, \cdots}) + \beta_s
\end{aligned}
$$

```{r}
amazon.stock = df %>% filter(Stock == "Amazon") %>% arrange(Date)
ggtsdisplay(amazon.stock %>% select(diff), main = "Difference of Price")
...
ggtsdisplay(apple.stock %>% select(diff), main = "Difference Price")
...
ggtsdisplay(google.stock %>% select(diff), main = "Difference of Price")
...
ggtsdisplay(facebook.stock %>% select(diff), main = "Difference of Price")
