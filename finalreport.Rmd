...
#####New version
new_df = read.csv("new_data.csv")
new_df$Date = as.Date(new_df$Date, format = "%Y/%m/%d")
new_df$Open = as.numeric(as.character(new_df$Open))
new_df$fb_close = as.numeric(as.character(new_df$fb_close))
new_df$google_close = as.numeric(as.character(new_df$google_close))
new_df$amazon_close = as.numeric(as.character(new_df$amazon_close))
new_df$apple_close = as.numeric(as.character(new_df$apple_close))
```
...
##new version
ggplot(data = new_df %>% arrange(Date), aes(x = Date, y = fb_close)) +
  geom_line() +
...
#FB new version
fb_lm = lm(fb_close ~ Open, data = new_df)
summary(fb_lm)
new_df$fb_naive.pred = predict(fb_lm, data=new_df$Open)
new_df$fb_naive.residual = new_df$fb_close - new_df$fb_naive.pred
...
###new version
facebook.ts = auto.arima(new_df %>% select(fb_close), xreg = new_df$Open, seasonal = TRUE)
facebook.ts %>% summary()
ggtsdisplay(facebook.ts$residuals, main = "ARIMA(2,0,2)")
# maybe add the 30 day trend?
try = Arima(new_df %>% select(fb_close), xreg = new_df$Open, order = c(2, 1, 2),seasonal = list(order = c(0, 0, 1),period = 30))
ggtsdisplay(try$residuals) #doesn't seem to change much
new_df = new_df %>% mutate(fb.ts.residuals = facebook.ts$residuals)

res.lm.model = lm(data = new_df, formula = fb.ts.residuals ~ 1)
res.lm.model %>% summary()

new_df = new_df %>% 
  mutate(fb.ts.pred = facebook.ts$fitted + res.lm.model$fitted.values)
new_df$arima_fb_residuals = c(new_df$fb_close - new_df$fb.ts.pred)

ggplot(data = new_df, aes(x = Date)) +
  geom_line(aes(y = fb_close, color = "red")) +
  geom_line(aes(y = fb.ts.pred, color = "blue")) +
  xlab("time") +
  ylab("Difference in Close Price and Open Price of Facebook Stock") +
  ggtitle("ARIMA(2,0,2)")

rmse(new_df$fb_close, new_df$fb_naive.pred)
rmse(new_df$fb_close, new_df$fb.ts.pred)
#forecast(facebook.ts, 30) %>% plot(xlim=c(1250, 1520))
plot(facebook.ts$residuals, ylab = "Residuals", main = "Residual Plot of ARIMA")
plot(try$residuals, ylab = "Residuals", main = "Residual Plot of ARIMA")
```
...
m_apple = rjags::jags.model(
    textConnection(gp_model_apple), 
    data = list(
      y = (apple.stock$Diff),
      d = dist(apple.stock$Date) %>% as.matrix(),
      N = length(apple.stock$Diff)
    ),
    quiet = TRUE
  )
  ...
  apple_cov_coda = rjags::coda.samples(
    m, variable.names=c("sigma2", "l"),
    n.iter=10000, thin=10
  )
  saveRDS(apple_cov_coda, file="gp_apple_jags.rds")

apple_gp = tidybayes::gather_samples(apple_cov_coda, sigma2, l)
...
```{r}
source("util.R")
facebook_subset = new_df[1291:1490, ]
fb_emp_cloud = facebook_subset %>% emp_semivariogram(fb_naive.residual,Date)

fb_emp = rbind(
  facebook_subset %>% emp_semivariogram(fb_naive.residual, Date, bin=TRUE, binwidth=1)  %>% mutate(binwidth="binwidth=1"),
  facebook_subset %>% emp_semivariogram(fb_naive.residual, Date, bin=TRUE, binwidth=5) %>% mutate(binwidth="binwidth=5"),
  facebook_subset %>% emp_semivariogram(fb_naive.residual, Date, bin=TRUE, binwidth=10) %>% mutate(binwidth="binwidth=10"),
  facebook_subset %>% emp_semivariogram(fb_naive.residual, Date, bin=TRUE, binwidth=15)   %>% mutate(binwidth="binwidth=15"),
  facebook_subset %>% emp_semivariogram(fb_naive.residual, Date, bin=TRUE, binwidth=30)  %>% mutate(binwidth="binwidth=30")
)
...
gp_exp_model = "model{
  ...
  sigma2_w ~ dnorm(10, 15) T(0,)
  sigma2   ~ dnorm(390, 400) T(0,)
  l        ~ dnorm(0.005, 1) T(0,) 
}"
```
...
if (file.exists("gp_jags.Rdata")) {
  ...
  m = rjags::jags.model(
    ...
    data = list(
      y = facebook_subset$fb_close,
      x = facebook_subset$Date,
      d = dist(facebook_subset$Date) %>% as.matrix(),
      N = nrow(facebook_subset),
      coef = coef(fb_lm)
    ),
    quiet = TRUE
  )
...
new_df$bayes_resid = new_df$fb_close - beta0 - beta1 * new_df$Open
new_df$fb_bayes_predict = beta0 + beta1 * new_df$Open
d_fit = data_frame(h=seq(0, 300, length.out = 1000)) %>% mutate(gamma = sq_exp_sv(h, sigma2, l, sigma2_w))
rmse(new_df$fb_close, new_df$fb_bayes_predict)
##can't make the code below work
rbind(
  new_df %>% emp_semivariogram(bayes_resid, x, bin=TRUE, binwidth=10, range_max=180) %>% mutate(bw = "binwidth=10"),
  new_df %>% emp_semivariogram(bayes_resid, x, bin=TRUE, binwidth=5, range_max=180) %>% mutate(bw = "binwidth=5")
) %>%
  ggplot(aes(x=h, y=gamma)) +
    geom_point() +
    ylim(0,NA) +
